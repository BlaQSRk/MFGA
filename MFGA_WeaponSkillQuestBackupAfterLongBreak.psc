...
Event OnQuestInit()
	;Runs when the quest first starts. Assigns the player to a variable, and registers them for future loadgames. Then runs initialization via ReInit().
...
;/
State busyQueueingExpDisplay
	Function QueueForExpDisplay (int indexValue)
		TraceAndNotify("Current State: " + GetState(), True)
		;/Utility.Wait(0.35) ;wait a bit so at the very least the first thread value can be set.
		if (skillDisplayQueue.Find(indexValue) < 0) ;If you can't find the indexValue in the queue, add it to the queue
			skillDisplayQueue.Add(indexValue)
			TraceAndNotify("Adding " + WeaponDataArray[indexValue].skillnameText + " to the queue.", True)
		else
			TraceAndNotify("Failed to add " + WeaponDataArray[indexValue].skillnameText + " to the queue. Reason: Value already in the queue.", True)	
...
Function GetWeaponType()
	...
	player.SetValue(MFGA_WeaponType, skillindex as float) ;Used in Perks to determine weapon type in a group. Useful for clumping
	player.SetValue(MFGA_AmmoType, ammoIndex as float) ;Used in Perks to determine weapon type in a group. Useful for clumping
...
Function GetWeaponTypeOld()
	TraceAndNotify("GetWeaponTypeOld()", True)
...
Function ReInit() ;Allows the game to reinitialize things if you need to change. OnQuestInit isn't sufficient. Put changes here.
	...
	HasNoReloadSpell[0] = 0
	HasNoReloadSpell[1] = 1
	HasNoReloadSpell[2] = 9
	HasNoReloadSpell[3] = 10
	HasNoReloadSpell[4] = 11
	HasNoReloadSpell[5] = 12
